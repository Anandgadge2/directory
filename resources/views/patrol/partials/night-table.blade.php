            ...
            <tbody>
                @forelse($patrols as $index => $p)
                    ...
                    <tr>
                        ...
                        <td>{{ $p->beat ?? 'NA' }}</td>
                        <td data-value="{{ $p->started_at }}">{{ \Carbon\Carbon::parse($p->started_at)->format('d M, H:i') }}</td>
                        <td class="patrol-hide-mobile" data-value="{{ $p->ended_at }}">
                             {{ $p->ended_at ? \Carbon\Carbon::parse($p->ended_at)->format('d M, H:i') : '-' }}
                        </td>
                        <td data-value="{{ $p->ended_at ? 1 : 0 }}">
                            @if($p->ended_at)
                        ...
                        <td class="patrol-hide-mobile text-center" data-value="{{ $p->speed ?? 0 }}">{{ number_format($p->speed ?? 0, 1) }}</td>
                    </tr>
                @empty
                    <tr>
